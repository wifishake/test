...
<article class="page-container">
  <form class="form form-horizontal"  action="{{ url('admin/user/doauth') }}" method="post">
   {{ csrf_field() }}
  ...
  <div class="row cl">
    ...
    <div class="formControls col-xs-8 col-sm-4 layui-input-inline">
        @foreach($roles as $v)
            @if( in_array($v->id,$own_rol) )
        <input type="checkbox" value="{{ $v->id }}" name="rol_id[]" lay-skin="primary" title="{{ $v->role_name }}" checked>{{ $v->role_name }}
            @else
         <input type="checkbox" value="{{ $v->id }}" name="rol_id[]" lay-skin="primary" title="{{ $v->role_name }}" >{{ $v->role_name }}
            @endif
        @endforeach
    </div>
  </div>
