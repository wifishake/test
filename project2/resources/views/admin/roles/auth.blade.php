...
<article class="page-container">
  <form class="form form-horizontal"  action="{{ url('admin/roles/doauth') }}" method="post">
   {{ csrf_field() }}
  ...
  <div class="row cl">
    ...
    <div class="formControls col-xs-8 col-sm-4 layui-input-inline">
        ...
        <input type="checkbox" value="{{ $v->id }}" name="perm_id[]" lay-skin="primary" title="{{ $v->node_name }}" checked>{{ $v->node_name }}
            @else
         <input type="checkbox" value="{{ $v->id }}" name="perm_id[]" lay-skin="primary" title="{{ $v->node_name }}" >{{ $v->node_name }}
            @endif
        @endforeach
    </div>
  </div>
